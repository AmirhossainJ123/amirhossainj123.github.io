...
function visible(num) {
	...
	check = toggle[num];
	if (!check) {
		document.getElementById("Keys" + (num + 1)).className =
			Curasses + " expandable";
		let Vis = All[num];
		toggle[num] = true;
		...
		for (y = 0; y < 10; y++) {
			...
			if (Exist) {
				...
				if (document.getElementById("input_" + x + "_" + y).checked)
					...
					Elements[x][1][y] = document.getElementById(
						"input_" + x + "_" + y
					).value;
				Exist = false;
				resize(x, y);
...
function resize(x, y) {
	...
	if (!(dude.type == "checkbox")) {
		...
		if (dude.type == "number") dude.style.width = dude.value.length + 3 + "ch";
	}
}
	...
	for (x = 0; x < Elements.length; x++) {
		...
		for (y = 0; y < Elements[x][0].length; y++) {
			if (Elements[x][0][y] == "<") {
				...
				if (innerstuff[0] == " ") current.className = "ctext spaced";
				button.append(current);
				if (innerstuff[innerstuff.length - 1] == " ") hasspace = true;
				else hasspace = false;
				innerstuff = "";
				hasseen = true;
			}
			...
			if (Elements[x][0][y] == ">") {
				if (hasseen) {
					...
					if (
						...
						if (hasspace) input.className = "chockbox spaced";
						else input.className = "chockbox";
					} else if (
						inputValue
							.replaceAll("<", "")
							.replaceAll(">", "")
						...
						if (hasspace) input.className = "inputfunn spaced";
						else input.className = "inputfunn";
					} else {
						if (hasspace) input.className = "inputfun spaced";
						else input.className = "inputfun";
					}
					if (!(Elements[x][1][num] == "" || Elements[x][1][num] == undefined))
						inputValue = Elements[x][1][num];
					input.checked = false;
					if (Elements[x][1][num] == "true") input.checked = true;
					else input.value = inputValue.replaceAll("<", "").replaceAll(">", "");
			...
			if (toggle) innerstuff += Elements[x][0][y];
			else inputValue += Elements[x][0][y];
		}
		var current = document.createElement("h3");
		current.id = "text_" + y;
		current.className = "ctext";
...
function DeleteElement(x) {
	...
	document.getElementById("Layer_" + x).style.animation =
		"Delete 0.3s ease-out 1 normal forwards";
	setTimeout(() => {
		refresh();
		inputset();
	}, 350);
...
function MoveUpElement(x) {
	document.getElementById("Layer_" + x.toString()).style.animation =
		"Moving 0.3s ease-out 1 normal forwards";
	document.getElementById("Layer_" + (x - 1).toString()).style.animation =
		"Moving 0.3s ease-out 1 normal forwards";
	temp = Elements[x - 1];
...
function MoveDownElement(x) {
	...
	document.getElementById(
		"Layer_" + parseInt(parseInt(x) + 1)
	).style.animation = "Moving 0.3s ease-out 1 normal forwards";
	temp = Elements[x + 1];
	Elements[x + 1] = Elements[x];
	Elements[x] = temp;
	setTimeout(() => {
...
function BuildItem(Index, Array) {
	...
	if (Info[6] == "1b") text += ",HideFlags:45";
	text += ",damage:" + Info[7] + "}";
	return text;
}
...
function Compiler(Info) {
	...
	for (let index = 0; index < Info.length; index++) {
		Index = index;
		if (Code[filled_lines] == undefined) Code[filled_lines] = "";
		if (Info[index][2][0] == 1) {
			if (Info[index][2][1] == 0) {
			...
			if (Info[index][2][1] == 2) {
				...
				Code[filled_lines] +=
					"summon " +
					Info[index][1][0].toLowerCase().replaceAll(" ", "_") +
					" ~" +
					Info[index][1][1] +
					" ~" +
					Info[index][1][2] +
			...
			if (Info[index][2][1] == 3) {
				...
				Code[filled_lines] +=
					"summon " +
					Definitions[SearchIndex(Definitions, Info[index][1][0])][1] +
					" ~" +
					Info[index][1][1] +
					" ~" +
					Info[index][1][2] +
					...
					Tags +
					"]," +
					BuildEntity(SearchIndex(Definitions, Info[index][1][0]), Definitions);
				filled_lines++;
			}
			if (Info[index][2][1] == 4) {
				Code[filled_lines] +=
					...
					Info[index][1][0] +
					" " +
					Info[index][1][2].replaceAll(" ", "_").toLowerCase() +
					" " +
					Info[index][1][1];
				filled_lines++;
			}
			if (Info[index][2][1] == 5) {
				Code[filled_lines] +=
					...
					Info[index][1][0] +
					" " +
					BuildItem(SearchIndex(Definitions, Info[index][1][2]), Definitions) +
					" " +
					Info[index][1][1];
				filled_lines++;
			}
			...
			if (Info[index][2][1] == 21) {
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][3]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run data merge entity @s {Motion:[${parseFloat(
					Info[index][1][0]
				).toString()},${parseFloat(Info[index][1][1]).toString()},${parseFloat(
					Info[index][1][2]
				).toString()}]}`;
				filled_lines++;
			}
			if (Info[index][2][1] == 22) {
				...
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][1]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] rotated as @s run tp @e[type=${Caser(
					Info[index][1][0]
						.replaceAll(" and ", ",")
						.replaceAll(" with ", ",")
						.replaceAll("&", "§")
				...
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_X run data get entity @s Pos[0] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_Y run data get entity @s Pos[1] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_Z run data get entity @s Pos[2] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][1]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] rotated as @s eyes run tp @e[type=${Caser(
					Info[index][1][0]
						.replaceAll(" and ", ",")
						.replaceAll(" with ", ",")
						.replaceAll("&", "§")
				...
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_X_PRIME run data get entity @s Pos[0] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_Y_PRIME run data get entity @s Pos[1] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result score @s CONST_MOTION_Z_PRIME run data get entity @s Pos[2] 10000`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players operation @s CONST_MOTION_X_PRIME -= @s CONST_MOTION_X`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players operation @s CONST_MOTION_Y_PRIME -= @s CONST_MOTION_Y`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players operation @s CONST_MOTION_Z_PRIME -= @s CONST_MOTION_Z`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result entity @s Motion[0] double 0.0001 run scoreboard players get @s CONST_MOTION_X_PRIME`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result entity @s Motion[1] double 0.0001 run scoreboard players get @s CONST_MOTION_Y_PRIME`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] store result entity @s Motion[2] double 0.0001 run scoreboard players get @s CONST_MOTION_Z_PRIME`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players reset @s CONST_MOTION_X`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players reset @s CONST_MOTION_Y`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players reset @s CONST_MOTION_Z`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players reset @s CONST_MOTION_X_PRIME`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players reset @s CONST_MOTION_Y_PRIME`;
				filled_lines++;
				Code[filled_lines] = SavedFilledLine;
				Code[filled_lines] += `execute as @e[type=${Caser(
					Info[index][1][0]
			...
			if (Info[index][2][1] == 28) {
				Code[filled_lines] += `effect give @e[type=${Caser(
					Info[index][1][0]
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] ${Info[index][1][1].tolowerCase()} ${Info[index][1][2]} ${
					Info[index][1][1]
				} true`;
				filled_lines++;
			}
			...
			if (Info[index][2][1] == 31) {
				...
				Code[
					filled_lines
				] += `summon area_effect_cloud ~ ~ ~ {Effects:[{Id:${SearchForPotionIndex(
					Info[index][1][0]
				)},Amplifier:${Info[index][1][1]},Duration:${
					Info[index][1][2] * 20
				}}],Particle:"${Info[index][1][3]}",Radius:${
					Info[index][1][4]
				},Tags:[${Tags}]}`;
				filled_lines++;
			}
		...
		if (Info[index][2][0] == 2) {
			if (Info[index][2][1] == 0) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_HEALTH";
				filled_lines++;
			}
			if (Info[index][2][1] == 1) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_FOOD";
				filled_lines++;
			}
			if (Info[index][2][1] == 2) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_DEATH";
				filled_lines++;
			}
			if (Info[index][2][1] == 3) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_DEALT";
				filled_lines++;
			}
			if (Info[index][2][1] == 4) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_TAKEN";
				filled_lines++;
			}
			if (Info[index][2][1] == 5) {
				Code[filled_lines] +=
					...
					" store result score " +
					Info[index][1][0] +
					" CONST_NUMBERS run scoreboard players get entity @s CONST_JUMP";
				filled_lines++;
			}
			if (Info[index][2][1] == 6) {
				let check = true;
		...
		if (Info[index][2][0] == 3) {
			...
			if (Info[index][2][1] == 1) {
				Code[
					filled_lines
				] += `scoreboard players operation ${Info[index][1][0]} CONST_NUMBERS += ${Info[index][1][1]} CONST_NUMBERS`;
				filled_lines++;
			}
			if (Info[index][2][1] == 2) {
				Code[
			...
			if (Info[index][2][1] == 5) {
				...
				Code[
					filled_lines
				] += `execute if score ${Info[index][1][1]} CONST_NUMBERS = ${Info[index][1][2]} CONST_NUMBERS run scoreboard players set ${Info[index][1][0]} CONST_NUMBERS 1`;
				filled_lines++;
				Code[filled_lines] = Chick;
				Code[
					filled_lines
				] += `execute unless score ${Info[index][1][1]} CONST_NUMBERS = ${Info[index][1][2]} CONST_NUMBERS run scoreboard players set ${Info[index][1][0]} CONST_NUMBERS 0`;
				filled_lines++;
			}
			if (Info[index][2][1] == 6) {
				Chick = Code[filled_lines];
			...
			if (Info[index][2][1] == 8) {
				Chick = Code[filled_lines];
				Code[filled_lines] += `execute if entity @e[type=${Caser(
					Info[index][1][1]
						.replaceAll(" and ", ",")
						.replaceAll(" with ", ",")
						.replaceAll("&", "§")
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players set ${Info[index][1][0]} CONST_NUMBERS 1`;
				filled_lines++;
				Code[filled_lines] = Chick;
				Code[filled_lines] += `execute unless entity @e[type=${Caser(
					Info[index][1][1]
						.replaceAll(" and ", ",")
						.replaceAll(" with ", ",")
						.replaceAll("&", "§")
						...
						.replaceAll(" is ", "=")
						.replaceAll(" ", "_")
				)}] run scoreboard players set ${Info[index][1][0]} CONST_NUMBERS 0`;
				filled_lines++;
			}
			if (Info[index][2][1] == 9) {
				Chick = Code[filled_lines];
				Code[filled_lines] += `execute if block ~${Info[index][1][1]} ~${
					Info[index][1][2]
				} ~${Info[index][1][3]} ${Info[index][1][4]
					.replaceAll(" ", "_")
					.toLowerCase()} run scoreboard players set ${
					Info[index][1][0]
				} CONST_NUMBERS 1`;
				filled_lines++;
				Code[filled_lines] = Chick;
				Code[filled_lines] += `execute unless block ~${Info[index][1][1]} ~${
					Info[index][1][2]
				} ~${Info[index][1][3]} ${Info[index][1][4]
					.replaceAll(" ", "_")
					.toLowerCase()} run scoreboard players set ${
			...
			if (Info[index][2][1] == 10) {
				Chick = Code[filled_lines];
				Code[filled_lines] += `execute if biome ~${Info[index][1][1]} ~${
					Info[index][1][2]
				} ~${Info[index][1][3]} ${Info[index][1][4]
					.replaceAll(" ", "_")
					.toLowerCase()} run scoreboard players set ${
				...
				filled_lines++;
				Code[filled_lines] = Chick;
				Code[filled_lines] += `execute unless biome ~${Info[index][1][1]} ~${
					Info[index][1][2]
				} ~${Info[index][1][3]} ${Info[index][1][4]
					.replaceAll(" ", "_")
					.toLowerCase()} run scoreboard players set ${
			...
			if (Info[index][2][1] == 11) {
				if (SearchIn(Definitions, Info[index][1][0]) == undefined)
					Definitions[Definitions.length] = [
						Info[index][1][0],
						Info[index][1][1].replaceAll(" ", "_").toLowerCase(),
						"",
						[],
						[],
						"0b",
					...
					Definitions[SearchIndex(Definitions, Info[index][1][0])] = [
						Info[index][1][0],
						Info[index][1][1].replaceAll(" ", "_").toLowerCase(),
						"",
						[],
			...
			if (Info[index][2][1] == 12) {
				Definitions[SearchIndex(Definitions, Info[index][1][0])][2] =
					Info[index][1][1];
			}
			if (Info[index][2][1] == 13) {
				Definitions[SearchIndex(Definitions, Info[index][1][0])][3][
					Definitions[SearchIndex(Definitions, Info[index][1][0])][3].length
				] = [Info[index][1][1], Info[index][1][2]];
			}
			if (Info[index][2][1] == 14) {
				Definitions[SearchIndex(Definitions, Info[index][1][0])][4][
					Definitions[SearchIndex(Definitions, Info[index][1][0])][4].length
				] = Info[index][1][1];
			}
			if (Info[index][2][1] == 15) {
				if ((Info[index][1][1] = "true"))
					Definitions[SearchIndex(Definitions, Info[index][1][0])][5] = "1b";
				else Definitions[SearchIndex(Definitions, Info[index][1][0])][5] = "0b";
			}
			if (Info[index][2][1] == 16) {
				if ((Info[index][1][1] = "true"))
					Definitions[SearchIndex(Definitions, Info[index][1][0])][6] = "1b";
				else Definitions[SearchIndex(Definitions, Info[index][1][0])][6] = "0b";
			}
			if (Info[index][2][1] == 17) {
				Definitions[SearchIndex(Definitions, Info[index][1][0])][7] =
					Info[index][1][1];
			}
			if (Info[index][2][1] == 18) {
				if (SearchIn(Definitions, Info[index][1][0]) == undefined)
			...
			if (Info[index][2][1] == 20) {
				...
				else Definitions[SearchIndex(Definitions, Info[index][1][0])][3] = "0b";
			}
			if (Info[index][2][1] == 21) {
				Definitions[SearchIndex(Definitions, Info[index][1][0])][4] = Info[
					index
				][1][1]
					.replaceAll(" ", "_")
					.toLowerCase();
			...
			if (Info[index][2][1] == 22) {
				...
				if (mainhand.replaceAll("nothing", "") == "")
					Definitions[SearchIndex(Definitions, Info[index][1][0])][5][0] =
						"air";
				else
					Definitions[SearchIndex(Definitions, Info[index][1][0])][5][0] =
						mainhand;
				if (offhand.replaceAll("nothing", "") == "")
					Definitions[SearchIndex(Definitions, Info[index][1][0])][5][1] =
						"air";
				else
					Definitions[SearchIndex(Definitions, Info[index][1][0])][5][1] =
						offhand;
			...
			if (Info[index][2][1] == 23) {
				...
				[boots, leggings, chestplate, helmet].forEach((bored) => {
					if (bored.replaceAll("nothing", "") == "")
						Definitions[SearchIndex(Definitions, Info[index][1][0])][6][
							current
						] = "air";
					else
						Definitions[SearchIndex(Definitions, Info[index][1][0])][6][
...
function Compile(Code) {
	return [Compiler(Code[0]), Compiler(Code[1])];
}
